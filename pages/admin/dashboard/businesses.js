...
export default function BusinessesPage() {
  ...
  const atLimit = !editingId && businesses.length >= MAX_BUSINESSES;

  return (
    <DashboardLayout title="Businesses">
      {atLimit ? (
        <div className="card mb-8 text-sm text-ivory/60">
          You're managing all {MAX_BUSINESSES} businesses on your plan. To add a new business, please contact support.
        </div>
      ) : (
        <form onSubmit={handleSubmit} className="card mb-8 grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
