...
//             </div>
//             <div>
//               <label className="text-xs text-ivory/50 mb-1.5 block">Sessions (Therapy Count)</label>
//               <input
//                 required
//                 type="number"
//                 min="1"
...
const emptyForm = { title: "", subtitle: "", therapyCount: "", price: "", discountPrice: "", isFeatured: false };
      ...
      {business && (
        <>
          <form onSubmit={handleSubmit} className="card mb-8 grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="text-xs text-ivory/50 mb-1.5 block">What's included (shown on card)</label>
              <input
                className="input-field"
                value={form.subtitle}
                onChange={(e) => setForm({ ...form, subtitle: e.target.value })}
                placeholder="e.g. 10 Sessions  or  5 Hair Cuts + Free Beard Trim"
              />
            </div>
            <div>
              <label className="text-xs text-ivory/50 mb-1.5 block">Session Count — optional, spa only</label>
              <input
                type="number"
                min="1"
                className="input-field"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {packages.map((p) => (
                <div key={p._id} className={`card text-center relative ${p.isFeatured ? "border-gold" : ""}`}>
                  ...
                  <div className="font-display text-lg text-ivory mt-2">{p.title}</div>
                  {p.subtitle && (
                    <div className="text-xs text-ivory/50 mt-1">{p.subtitle}</div>
                  )}
                  <div className="my-2">
                    {p.discountPrice ? (
                      <>
